    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr class="{{ $loop->index % 2 == 0 ? 'bg-light' : 'bg-secondary text-white' }}">
                ...
                <td>{{ $reunion->hora }}</td>
                <td>
                    <!-- Mostrar el estado actual de la reunión -->
                ...
                <td>
                    <!-- Formulario para aceptar la reunión -->
                    @if($reunion->estado == 'pendiente')
                    <form method="POST" action="{{ route('profesor.reuniones.actualizar', $reunion->id) }}" style="display:inline;">
                        @csrf
                        <button type="submit" name="estado" value="aceptada" class="btn btn-success btn-sm">Aceptar</button>
                    </form>
                    @endif

                    <!-- Formulario para rechazar la reunión -->
                    @if($reunion->estado == 'pendiente')
                    <form method="POST" action="{{ route('profesor.reuniones.actualizar', $reunion->id) }}" style="display:inline;">
                        @csrf
